...
void Node::setEnlightValue(int start_x,int start_y,int destination_x, int destination_y)
{
	if (obstacle_ == true)
	{
		return;
	...
	x1 = destination_x;
	y1 = destination_y;
	enlighten_value_ = sqrt((x_ - x1) * (x_ - x1) + (y_ - y1) * (y_ - y1)) + sqrt((x_ - x0) * (x_ - x0) + (y_ - y0) * (y_ - y0));
}
